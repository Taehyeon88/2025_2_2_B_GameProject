...
public class MazeGenerator : MonoBehaviour
{
    ...
    private void GenerateWithDFS()             //DFS 알고리즘으로 생성
    {
    ...
    private void CreateCells()           //셀 생성 함수
    {
        if (cellPrefab == null)
        {
            Debug.Log("셀 프리팹이 없음");
    ...
    private void RemoveWallBetween(MazeCell current, MazeCell next)    //두셀 사이의 벽제거
    {
    ...
    private IEnumerator GenerateWithDFSVisualized()
    {
        ...
        while (cellStack.Count > 0)
        {
            ...
            if (unvisitedNeighbors.Count > 0)
            {
                ...
                current.SetColor(visitedColor);             //현재 칸 방문 완료 색으로 설정
                visitedCount++;
                next.visited = true;
                cellStack.Push(next);
